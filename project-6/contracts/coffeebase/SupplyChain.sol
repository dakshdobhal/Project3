...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole{
  ...
  function harvestItem(uint _upc, address _originFarmerID, string _originFarmName, string _originFarmInformation, string  _originFarmLatitude, string  _originFarmLongitude, string  _productNotes, uint _productID) public onlyFarmer
  {
  ...
  function packItem(uint _upc) public processed(_upc) onlyFarmer verifyCaller(items[_upc].originFarmerID){
    // Update the appropriate fields
  ...
  itemState = uint(items[_upc].itemState); //to avoid error Type enum SupplyChain.State is not implicitly convertible to expected type uint256.
